...
export default function BolumDuzenle({ params }) {
  const { id, bolumId } = use(params); // Senin klasör yapındaki parametreler
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black opacity-10 text-5xl italic">YUKLENIYOR</div>;
    ...
    <div className="min-h-screen py-24 px-6 bg-[#fcfcfc] dark:bg-[#080808]">
      ...
      <div className="max-w-3xl mx-auto">
        <header className="mb-16 text-center">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 italic">Eserini Mükemmelleştir</p>
        </header>
        ...
        <form onSubmit={handleUpdate} className="space-y-8 bg-white dark:bg-black/20 p-10 rounded-[3rem] border dark:border-white/5 shadow-xl shadow-black/5">
          <div>
            <label className="block text-[9px] font-black uppercase tracking-widest text-gray-400 mb-3 ml-4">Bölüm Başlığı</label>
            <input 
              required
          ...
          <div>
            <label className="block text-[9px] font-black uppercase tracking-widest text-gray-400 mb-3 ml-4">İçerik</label>
            <textarea 
              required
              rows="15"
          ...
          <div className="flex gap-4">
            <button 
              type="button" 
              onClick={() => router.back()}
              className="flex-1 h-14 rounded-full bg-gray-100 dark:bg-white/5 text-[10px] font-black uppercase tracking-widest text-gray-400"
            >
              Vazgeç
            </button>
            <button 
              type="submit" 
              disabled={updating}
              className="flex-[2] h-14 rounded-full bg-black dark:bg-white text-white dark:text-black text-[10px] font-black uppercase tracking-widest shadow-xl shadow-red-600/10 hover:bg-red-600 transition-all"
            >
