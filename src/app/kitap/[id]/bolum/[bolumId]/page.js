...
export default function BolumDetay({ params }) {
  ...
  useEffect(() => {
    async function getFullData() {
      ...
      try {
        // OKUNMA HATASINI YAKALAMA
        const { error: rpcError } = await supabase.rpc('increment_views', { target_chapter_id: bolumId });
        if (rpcError) {
          console.error("OKUNMA HATASI (RPC):", rpcError.message);
        } else {
          console.log("OKUNMA DURUMU: Başarıyla artırıldı.");
        }
  ...
  // Çökmeyi engelleyen içerik kontrolü
      ...
      <div className="flex justify-center min-h-screen relative">
        <main className="w-full max-w-2xl pt-48 pb-40 px-8 shrink-0">
                    ...
                    <button 
                      ...
                      className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-full transition-all border
                        ${count > 0 || activePara === paraId
                          ...
                          : 'bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800 text-gray-300 opacity-0 group-hover:opacity-100'
                        }`}
                    >
          ...
          <footer className="mt-40 flex items-center justify-between gap-6 border-t dark:border-white/5 pt-10">
            {prevChapter ? (
              <Link href={`/kitap/${id}/bolum/${prevChapter.id}`} className="flex-1 h-12 flex items-center justify-center rounded-full bg-gray-100 dark:bg-white/5 text-[9px] font-black uppercase tracking-widest text-gray-400">Önceki</Link>
            ...
            ) : <div className="flex-1 h-12 flex items-center justify-center rounded-full bg-green-600/10 text-green-600 text-[9px] font-black uppercase">Son</div>}
          </footer>
        </main>
        ...
        <aside className={`fixed top-24 right-8 bottom-8 w-[350px] transition-all duration-500 z-50 
          ...
          <div className="h-full bg-white dark:bg-[#0f0f0f] border dark:border-white/10 rounded-[2.8rem] shadow-2xl flex flex-col overflow-hidden">
            <div className="p-6 flex justify-between items-center border-b dark:border-white/5">
              <span className="text-[9px] font-black uppercase opacity-40">Yorumlar</span>
              <button onClick={() => setActivePara(null)} className="text-gray-400 hover:text-red-600">✕</button>
            </div>
