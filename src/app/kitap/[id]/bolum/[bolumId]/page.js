...
export default function BolumDetay({ params }) {
  ...
  useEffect(() => {
    async function getFullData() {
      ...
      try {
        // OKUNMA SAYISI FIX: RPC hatasını engellemek için bolumId'yi metne çevirip yolluyoruz
        await supabase.rpc('increment_views', { target_chapter_id: String(bolumId) });
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black opacity-10 animate-pulse text-5xl italic tracking-tighter uppercase">YAZIO</div>;

  // BUTONLARIN ÇALIŞMAMA FIX'İ: ID'leri string olarak karşılaştırıyoruz ki index -1 çıkmasın
  const currentIndex = data.allChapters.findIndex(c => String(c.id) === String(bolumId));
    ...
    <div className="min-h-screen bg-[#fdfdfd] dark:bg-[#0a0a0a]">
      ...
      <div className="flex justify-center min-h-screen relative">
        <main className="w-full max-w-2xl pt-48 pb-40 px-8 shrink-0">
                    ...
                    <button 
                      ...
                      className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-full transition-all border
                        ${count > 0 || activePara === paraId
                          ? 'bg-red-600 border-red-600 text-white opacity-100 shadow-lg shadow-red-600/20' 
          ...
          {/* BUTONLAR FIX: prevChapter kontrolü eklendi */}
          <footer className="mt-40 flex items-center justify-between gap-6 border-t dark:border-white/5 pt-10">
            {prevChapter ? (
              <Link href={`/kitap/${id}/bolum/${prevChapter.id}`} className="flex-1 h-12 flex items-center justify-center rounded-full bg-gray-100 dark:bg-white/5 text-[9px] font-black uppercase tracking-widest text-gray-400 hover:bg-gray-200 border dark:border-white/5 transition-all">
                Önceki Bölüm
              </Link>
            ...
            {nextChapter ? (
              ...
              <div className="flex-1 h-12 flex items-center justify-center rounded-full bg-green-600/10 text-green-600 text-[9px] font-black uppercase border border-green-600/20">
                Eserin Sonu
              </div>
            )}
          </footer>
        ...
        <aside className={`fixed top-24 right-8 bottom-8 w-[350px] transition-all duration-500 z-50 
          ...
          <div className="h-full bg-white dark:bg-[#0f0f0f] border dark:border-white/10 rounded-[2.8rem] shadow-2xl flex flex-col overflow-hidden">
            <div className="p-6 flex justify-between items-center border-b dark:border-white/5">
              ...
              <button onClick={() => setActivePara(null)} className="text-gray-400 hover:text-red-600 transition-all text-xl">✕</button>
            </div>
