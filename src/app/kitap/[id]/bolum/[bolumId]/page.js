      ...
      <div className="flex justify-center min-h-screen relative">
        <main className="w-full max-w-2xl pt-48 pb-40 px-8 shrink-0">
              ...
              return (
                <div key={i} className="relative mb-10 group">
                  <div className={`flex items-start gap-4 transition-all duration-500 rounded-[1.8rem] p-4 -mx-4 ${activePara === paraId ? 'bg-red-50/50 dark:bg-red-950/10 ring-1 ring-red-100 dark:ring-red-900/20' : ''}`}>
                    ...
                    {/* YORUM SAYISI FIX: count > 0 ise her zaman görünür, değilse hover ile çıkar */}
                    <button 
          ...
          <footer className="mt-40 flex items-center justify-between gap-6 border-t dark:border-white/5 pt-10">
            ...
            {nextChapter ? (
              <Link href={`/kitap/${id}/bolum/${nextChapter.id}`} className="flex-1 h-12 flex items-center justify-center rounded-full bg-black dark:bg-white text-white dark:text-black text-[9px] font-black uppercase tracking-widest shadow-xl shadow-red-600/10 hover:bg-red-600 transition-all">Sonraki Bölüm</Link>
            ) : (
