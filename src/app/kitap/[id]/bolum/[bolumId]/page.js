...
export default function BolumDetay({ params }) {
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] dark:bg-[#080808]">
      ...
      {isChapterListOpen && (
        <div
          className="fixed inset-0 z-[300] flex items-center justify-center md:p-6"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div className="fixed left-1/2 -translate-x-1/2 top-[calc(1rem_+_env(safe-area-inset-top))] bottom-[calc(1rem_+_env(safe-area-inset-bottom))] w-[calc(100vw_-_2rem)] md:relative md:inset-auto md:translate-x-0 md:w-full md:max-w-lg md:max-h-[82vh] rounded-[1.6rem] md:rounded-[2rem] bg-[#fcfcfc] dark:bg-[#101010] border border-black/5 dark:border-white/10 shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-8 md:zoom-in-95 duration-300">
            ...
            <div className="shrink-0 px-4 pt-2.5 pb-2.5 md:p-7 border-b border-black/5 dark:border-white/10">
              <div className="flex items-start justify-between gap-5">
                <div className="min-w-0">
                  <p className="mb-0.5 text-[8px] md:text-[9px] font-black uppercase tracking-[0.24em] text-red-600">
                    {visibleChapters.length} Bölüm
                  </p>
                  <h2 id="chapter-list-title" className="truncate text-base md:text-2xl font-black tracking-tight text-gray-950 dark:text-white">
                    {data.book?.title}
                  </h2>
                  <p className="mt-0.5 hidden md:block truncate text-xs font-medium text-gray-400">
                ...
                <button
                  type="button"
                  onClick={() => setIsChapterListOpen(false)}
                  className="shrink-0 grid place-items-center w-8 h-8 md:w-10 md:h-10 rounded-full bg-black/5 dark:bg-white/10 text-sm text-gray-500 dark:text-gray-300 hover:bg-red-600 hover:text-white transition-all"
                  aria-label="Kapat"
                >
                  ✕
                </button>
                ...
                {visibleChapters.map((chapter, index) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`group flex items-center gap-2.5 md:gap-4 rounded-xl md:rounded-2xl px-3 py-2 md:px-4 md:py-4 border transition-all ${isCurrent
                    ...
                    >
                      <span className={`shrink-0 grid place-items-center w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl text-[10px] md:text-xs font-black tabular-nums ${isCurrent
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2">
                          <span className="truncate text-[13px] md:text-sm font-bold">{chapter.title}</span>
                          {chapter.is_draft && (
                        ...
                        <span className={`mt-0.5 block text-[8px] md:text-[9px] font-black uppercase tracking-[0.18em] ${isCurrent ? 'text-white/70' : 'text-gray-400'}`}>
      ...
      <div className="flex justify-center min-h-screen relative">
       ...
       {authorProfile && (
            <div className="mt-8 flex items-center justify-between gap-4 p-5 rounded-2xl bg-current/5 transition-all border border-current/10">
              <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                <Link href={authorLink} className="flex items-center gap-3 md:gap-4 min-w-0 group/author">
                  ...
                  <div className="flex flex-col shrink min-w-0">
                    ...
                    <div className="text-sm font-bold group-hover/author:text-red-600 transition-colors flex items-center gap-x-1.5">
                      <Username
                ...
                {coAuthorProfile && (
                  <Link 
                    ...
                    className="flex items-center gap-3 md:gap-4 min-w-0 group/coauthor"
                  >
                    <div className="text-sm font-bold group-hover/coauthor:text-red-600 transition-colors">
                      <Username
                    ...
                    <div className={`w-12 h-12 rounded-full overflow-hidden shrink-0 border-2 border-transparent group-hover/coauthor:border-red-600 animate-in fade-in slide-in-from-right-2 duration-500 transition-all ${readerSettings.theme.includes('bg-[#f4ecd8]')
                      ? 'bg-[#e8d9c3]'
                      : readerSettings.theme.includes('bg-[#0a0a0a]')
                        ? 'bg-white/10'
                        : 'bg-gray-200'
              ...
              <div className="text-red-600 opacity-20 shrink-0 ml-1">→</div>
            </div>
          )}
        </main>
