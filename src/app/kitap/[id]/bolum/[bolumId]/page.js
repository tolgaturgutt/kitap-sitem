...
export default function BolumDetay({ params }) {
  ...
  useEffect(() => {
    async function getFullData() {
      ...
      try {
        ...
        // OKUMA GEÇMİŞİ KAYDI (BURAYA EKLENDİ)
  ...
  // ... (Geri kalan tüm görsel kodların ve butonların olduğu gibi kalsın) ...
      ...
      <div className="flex justify-center min-h-screen relative">
        <main className="w-full max-w-2xl pt-48 pb-40 px-8 shrink-0">
              ...
              return (
                <div key={i} className="relative mb-10 group">
                  <div className={`flex items-start gap-4 transition-all duration-500 rounded-[1.8rem] p-4 -mx-4 ${activePara === paraId ? 'bg-red-50/50 dark:bg-red-950/10' : ''}`}>
                    ...
                    <button onClick={() => setActivePara(activePara === paraId ? null : paraId)} className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-full transition-all border ${count > 0 || activePara === paraId ? 'bg-red-600 border-red-600 text-white shadow-lg' : 'bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800 text-gray-300 opacity-0 group-hover:opacity-100 hover:border-red-600'}`}>
          ...
          <footer className="mt-40 flex items-center justify-between gap-6 border-t dark:border-white/5 pt-10">
            {prevChapter ? ( <Link href={`/kitap/${id}/bolum/${prevChapter.id}`} className="flex-1 h-12 flex items-center justify-center rounded-full bg-gray-100 dark:bg-white/5 text-[9px] font-black uppercase tracking-widest text-gray-400 hover:bg-gray-200 transition-all">Önceki Bölüm</Link> ) : <div className="flex-1" />}
            {nextChapter ? ( <Link href={`/kitap/${id}/bolum/${nextChapter.id}`} className="flex-1 h-12 flex items-center justify-center rounded-full bg-black dark:bg-white text-white dark:text-black text-[9px] font-black uppercase tracking-widest shadow-xl hover:bg-red-600 transition-all">Sonraki Bölüm</Link> ) : ( <div className="flex-1 h-12 flex items-center justify-center rounded-full bg-green-600/10 text-green-600 text-[9px] font-black uppercase border border-green-600/20 text-center">Eserin Sonu</div> )}
          </footer>
        </main>
