...
// --- GÜNCELLENMİŞ PODYUM TASARIMI (BALANSLI COMPACT - TAM ORTASI) ---
function LeaderboardSection({ title, icon, colorClass, data, type, adminEmails }) {
  ...
  const rankStyles = {
    0: { // 1. SIRA (ALTIN)
      ...
      crown: "👑",
      // MOBİLDE SCALE YOK, SADECE PC'DE HAFİF VAR
      scale: "scale-100 md:scale-105 z-10", 
      // Avatar: w-12 (48px) - Ne çok büyük ne çok küçük
    ...
    1: { // 2. SIRA (GÜMÜŞ)
      ...
      crown: "🥈",
      scale: "scale-100 z-0",
      // Avatar: w-10 (40px) - Altına yakın boyutta
      avatarSize: "w-10 h-10 md:w-16 md:h-16",
      marginBottom: "mb-3 md:mb-6"
    },
    2: { // 3. SIRA (BRONZ)
  ...
  return (
    <div className="bg-white dark:bg-[#0a0a0a] rounded-[1.5rem] p-3 md:p-6 border border-gray-100 dark:border-white/5 shadow-2xl relative overflow-hidden flex flex-col h-full">
       ...
       <h2 className="text-sm md:text-xl font-black uppercase tracking-tight mb-4 md:mb-8 flex items-center gap-2 relative z-10">
          <span className="text-xl md:text-3xl">{icon}</span>
          <div className="leading-tight text-xs md:text-xl">{title}</div>
       </h2>

       {/* --- PODYUM ALANI --- */}
       {/* min-h düşürüldü, boşluklar kısıldı */}
       <div className="flex items-end justify-center gap-1.5 md:gap-4 mb-4 md:mb-8 min-h-[150px] md:min-h-[200px]">
             ...
             return (
                <div key={item.userId} className={`flex-1 flex flex-col items-center text-center transition-all duration-300 min-w-0 ${style.scale}`}>
                   ...
                   <div className="relative mb-1.5 md:mb-3">
                      <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-white dark:bg-black border border-gray-100 dark:border-white/10 px-1.5 py-0 rounded-full shadow-md text-[9px] z-20 whitespace-nowrap">
                         {style.crown}
                      </div>
                   ...
                   {/* İsim Kutusu - SABİT BOY (h-70px) */}
                   {/* Ne çok basık ne çok uzun, tam ideal */}
                   <div className={`w-full h-[70px] md:h-[110px] rounded-t-lg md:rounded-t-2xl bg-gradient-to-b ${style.gradient} border-t-2 ${style.border} flex flex-col justify-center items-center shadow-lg backdrop-blur-sm px-0.5`}>
                      ...
                      <div className="w-full flex items-center justify-center flex-1 overflow-hidden px-1">
                        <Link href={`/yazar/${item.username}`} className="block w-full">
                           <Username 
                              ...
                              isAdmin={isUserAdmin} 
                              // Fontlar: Mobilde 10px (Okunur) ve 9px (Diğerleri)
                              // justify-center mx-auto eklendi (Ortalama garantisi)
                              className={`
          ...
          {others.map((item, idx) => {
             ...
             return (
                <Link href={`/yazar/${item.username}`} key={item.userId} className="flex items-center gap-2 md:gap-3 group p-1.5 hover:bg-gray-50 dark:hover:bg-white/5 rounded-lg transition-all border border-transparent hover:border-gray-200 dark:hover:border-white/10">
                   <div className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center font-bold text-gray-400 bg-gray-100 dark:bg-white/5 rounded text-[9px] md:text-xs">#{realRank}</div>
