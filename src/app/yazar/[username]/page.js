      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-8 md:mb-12 flex flex-col md:flex-row items-center gap-6 md:gap-10 bg-white dark:bg-white/5 p-6 md:p-10 rounded-3xl md:rounded-[4rem] border dark:border-white/5 shadow-sm">
          ...
          <div className="flex-1 text-center md:text-left w-full">
            <div className="flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4 mb-3 md:mb-4">
              <div>
        ...
        <div className="min-h-[300px]">
          {activeTab === 'hakkında' ? (
            <div className="p-6 md:p-8 bg-white dark:bg-white/5 rounded-2xl md:rounded-3xl border dark:border-white/5 italic text-gray-500 leading-relaxed whitespace-pre-line text-sm md:text-base">{author.bio || "Biyografi henüz eklenmemiş."}</div>
          ) : activeTab === 'panolar' ? (
            <div className="space-y-4 md:space-y-6">
                ...
                <div className="mb-8 md:mb-10">
                  {!showAddPano ? (
                    <button 
                      ...
                      className="w-full p-6 md:p-8 bg-red-600/5 dark:bg-red-600/10 border-2 border-dashed border-red-600/20 rounded-2xl md:rounded-[2.5rem] flex items-center justify-center gap-3 md:gap-4 hover:bg-red-600/10 transition-all group"
                    >
                    ...
                    <div className="bg-white dark:bg-white/5 p-6 md:p-8 rounded-2xl md:rounded-[3rem] border-2 border-red-600 shadow-2xl animate-in zoom-in-95">
                      ...
                      <form onSubmit={handleAddPano} className="space-y-3 md:space-y-4">
                        <input value={newPanoTitle} onChange={e => setNewPanoTitle(e.target.value)} placeholder="Pano Başlığı..." className="w-full p-3 md:p-4 bg-gray-50 dark:bg-black border dark:border-white/10 rounded-xl md:rounded-2xl outline-none focus:border-red-600 text-sm" />
                        <textarea value={newPanoContent} onChange={e => setNewPanoContent(e.target.value)} placeholder="Ne hakkında yazmak istersin..." className="w-full p-3 md:p-4 h-32 md:h-40 bg-gray-50 dark:bg-black border dark:border-white/10 rounded-xl md:rounded-2xl outline-none focus:border-red-600 resize-none text-sm" />
                        ...
                        <div className="flex gap-2 md:gap-3 pt-2 md:pt-4">
                          <button type="button" onClick={() => setShowAddPano(false)} className="flex-1 py-3 md:py-4 bg-gray-100 dark:bg-white/5 rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black uppercase">İptal</button>
                          <button type="submit" disabled={isSubmitting} className="flex-[2] py-3 md:py-4 bg-red-600 text-white rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black uppercase shadow-lg shadow-red-600/30">{isSubmitting ? 'Paylaşılıyor...' : 'Paylaş 🚀'}</button>
                        </div>
                      </form>
                    </div>
                  )}
              ...
              {panos.length === 0 ? <p className="text-center py-12 md:py-20 text-gray-500 italic uppercase text-[9px] md:text-[10px] tracking-widest">Henüz pano yok.</p> : panos.map(pano => (
                <div 
                  key={pano.id} 
                ...
                >
                  ...
                  <div className="flex-1">
                    <h3 className="text-base md:text-xl font-black dark:text-white mb-1 md:mb-2 line-clamp-1 uppercase tracking-tighter">{pano.title}</h3>
                  ...
                  {(isAdmin || isOwner) && (
                    <button 
                      ...
                      className="absolute top-4 right-4 md:top-6 md:right-6 px-2.5 md:px-3 py-1 bg-red-600 text-white rounded-full text-[8px] md:text-[9px] font-black uppercase opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      SİL
                    </button>
                  )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-6">
              {books.map(k => (
                <Link key={k.id} href={`/kitap/${k.id}`} className="group flex flex-col">
                  ...
                  <h3 className="text-[9px] md:text-[10px] font-black text-center uppercase truncate italic">{k.title}</h3>
