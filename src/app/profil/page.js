  ...
  async function handleBannerConfirm(transforms) {
    ...
    try {
      ...
      const [desktopCrop, mobileCrop] = await Promise.all([
        ...
        cropProfileBanner(sourceFile, transforms.mobile, { width: 1080, height: 1300, fileName: 'mobile.jpg' })
      ]);
      ...
      const [desktopFile, mobileFile, preservedSource] = await Promise.all([
        ...
        imageCompression(mobileCrop, { ...compressionOptions, maxWidthOrHeight: 1300 }),
                  ...
                  {canUsePlusFeatures && (
                    <div className="md:col-span-2 mb-2 p-4 bg-amber-50 dark:bg-amber-500/5 rounded-2xl md:rounded-3xl border border-dashed border-amber-300 dark:border-amber-500/30 text-center relative group hover:bg-amber-100 dark:hover:bg-amber-500/10 transition-colors">
                      ...
                      <p className="mt-1 text-[8px] font-bold uppercase tracking-widest text-amber-600/60">
                        PC: 2048 × 1152 · Mobil: 1080 × 1300 · Plus özellik
                      </p>
                      {profileData.banner_url && (
        ...
        <div className="min-h-[300px]">
          {activeTab === 'rozetler' ? (
            ...
            <div className="space-y-4 md:space-y-6">
              {myPanos.length === 0 ? (
                <div className="text-center py-8 md:py-10 text-sm text-gray-500"> Gösterilecek panon yok.</div>
              ) : (
                myPanos.map(pano => (
                  <div
                    key={pano.id}
