...
import { Capacitor } from '@capacitor/core'; // 👈 Bu satır APK kontrolü yapar
...
export default function Footer() {
  ...
  // 👇 APK KONTROLÜ İÇİN STATE
  const [isApp, setIsApp] = useState(false);

useEffect(() => {
    // 1. Capacitor nesnesi var mı? (En garanti yöntem)
    const isCapacitor = typeof window !== 'undefined' && window.Capacitor;
    
    // 2. Veya Platform Native mi?
    const isNative = Capacitor.isNativePlatform();

    // 3. Veya User Agent içinde 'wv' (WebView) yazıyor mu?
    // (Android uygulamaları genelde 'wv' etiketi taşır)
    const userAgent = window.navigator.userAgent.toLowerCase();
    const isWebView = userAgent.includes('wv');

    // Herhangi biri 'Evet' diyorsa bu bir uygulamadır
    if (isCapacitor || isNative || isWebView) {
      setIsApp(true);
    }
  }, []);

  // 👇 EĞER UYGULAMAYSA (APK), HİÇBİR ŞEY GÖSTERME (Footer'ı yok et)
  if (isApp) return null;

  return (
    // 👇 BURAYA DİKKAT: 'hidden md:block' silindi. 
    // Artık normal 'w-full' var çünkü mobil sitede gözüksün istiyoruz.
    // APK'da zaten yukarıdaki 'if (isApp)' kodu burayı engelleyecek.
